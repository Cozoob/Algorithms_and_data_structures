...
def euler(G):
    ...
    n = len(G)
    path = []
    last_visited = [-1 for _ in range(n)]
    leave_vertex = [0 for _ in range(n)]
    ...
    if min(leave_vertex) != 0 or max(leave_vertex) != 0 or min(last_visited) == -1:
        return None
    return path
